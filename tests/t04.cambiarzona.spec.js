...
test('Verificación que la búsqueda se recargue al cambiar zonas', async ({ page }) => {
  ...
  const zonaUnoValor = await page.locator('[data-testid="zona-input"]').inputValue()
  ...
  await segunda.click()
  await page.waitForResponse('**/api/comercios/lista?**');
  const zonaDosValor = await page.locator('[data-testid="zona-input"]').inputValue()
  ...
  await tercera.click()
  await page.waitForResponse('**/api/comercios/lista?**');
  const zonaTresValor = await page.locator('[data-testid="zona-input"]').inputValue()
