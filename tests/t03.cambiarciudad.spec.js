...
test('Verificación que la búsqueda se recargue al cambiar ciudades', async ({ page }) => {
  await page.goto('/');
  ...
  const ciudadUnoValor = await page.locator('[data-testid="ciudad-input"]').inputValue()
  ...
  const textoTitulo1 = await page.locator('[data-testclass="tarjeta-title"]').first().textContent();

  await page.locator('[data-testid="ciudad-pulldown-button"]').click()
  await segunda.click()
  await page.waitForResponse('**/api/comercios/lista?**');
  const ciudadDosValor = await page.locator('[data-testid="zona-input"]').inputValue()
  ...
  const textoTitulo2 = await page.locator('[data-testclass="tarjeta-title"]').first().textContent();
  ...
  await tercera.click()
  await page.waitForResponse('**/api/comercios/lista?**');
  const ciudadTresValor = await page.locator('[data-testid="ciudad-input"]').inputValue()
  ...
  const textoTitulo3 = await page.locator('[data-testclass="tarjeta-title"]').first().textContent();
